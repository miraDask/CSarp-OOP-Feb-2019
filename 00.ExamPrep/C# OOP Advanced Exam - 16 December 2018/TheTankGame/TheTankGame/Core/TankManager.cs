    ...
    public class TankManager : IManager
    {
        ...
        public string AddVehicle(IList<string> arguments)
        {
            ...
            IVehicle vehicle = this.vehicleFactory
                .CreateVehicle(vehicleType, model, weight, price, attack,defense, hitPoints);
        ...
        public string AddPart(IList<string> arguments)
        {
            ...
            //        break;
            //}
            var methodName = "Add" + part.GetType().Name;
            var method = this.vehicles[vehicleModel].GetType().GetMethods().FirstOrDefault(x => x.Name == methodName);

            method.Invoke(this.vehicles[vehicleModel], new object[] { part});
            //if (part != null)
            //{
            //    if (partType == "Arsenal")
            //    {
